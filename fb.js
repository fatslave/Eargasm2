...
function share_playlist(playlistId) {
  FB.ui({
      method: 'stream.publish',
      attachment: {
        name: "Check this playlist out!",
...
function videoTicker(artist, track , videoId)
{
	...
	var url = encodeURIComponent('http://eargasm.xtreemhost.com/index.php?type=music&title='+title);
	console.log(url);
	FB.api('me/fatslave:Catch?propa='+escape(artist)+'&propb='+escape(track)+'&propc='+videoId+'&music=' + encodeURIComponent('http://eargasm.xtreemhost.com/index.php?type=music&title='+title), 'post', function (response) {
...
function doRedirect(){
	...
	var actionID = window.location.href.split('=')[3].split('&')[0];
		// do redirect	
	
	//console.log(actionID);
	
	var url = 'https://graph.facebook.com/'+ actionID + '?access_token=' + FB.getAccessToken();
	$.ajax({
		url:url,
		type:'GET',
		dataType:'json',
